...
train_cows.extend(cows[ind+1:])

valid_cows.append(cows[ind])
...
model = models.googlenet(pretrained=False, num_classes = 6).to(device)
#final_in_features = model.fc.in_features
#model.fc = nn.Linear(final_in_features, 6).to(device)
...
def iterate(ep, mode):
    ...
    for it in monitor:
        if decode:
            ...
            loss = out_criterion(out[0], lbl.to(device))
            preds = out[0]

        num_samples += lbl.size(0)
        run_loss += loss.item() * lbl.size(0)
        run_err += ((preds.detach().cpu() - lbl) ** 2).view(-1, 3, 2).sum(2).sqrt().sum(0)
...
for epoch in range(num_epochs):
    ...
    print('Epoch ', epoch+1, ' error is: ', err)
    ...
    with torch.no_grad():
        ...
        if patience > 8:
           torch.save(model, '/home/vahidn/projects/def-banire/vahidn/CLT/GoogleNet15.pt')
           break
        valid_loss.append(l)
    ...
    print('Valid error in epoch ', epoch+1, ' is: ', err)
...
torch.save(model, '/home/vahidn/projects/def-banire/vahidn/CLT/GoogleNet15.pt')
print('Best epoch : ', best_ep, 'Best avg is: ', best_avg)
