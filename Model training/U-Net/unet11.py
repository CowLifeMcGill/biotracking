...
class Block(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.in_ch = in_ch
        self.out_ch = out_ch
        self.conv1 = nn.Conv2d(self.in_ch, self.out_ch, 3)
        self.relu  = nn.ReLU()
        self.conv2 = nn.Conv2d(self.out_ch, self.out_ch, 3)
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.enc_blocks:
            ...
            x = self.pool(x)
        x = self.avg(x)    
        y = self.fc(x.view(-1, 1024))
        return y

model = UNet().to(device)
                                                                                                                                                                                                                                                
# model = segnet.SegNet(channels=cfg['channels'], decode=cfg['decode']).to(device)
#model = models.vgg16(pretrained=False, num_classes=num_classes).to(device)
...
for epoch in range(num_epochs):
    ...
    if patience > 8:
       torch.save(model, '/home/vahidn/projects/def-banire/vahidn/CLT/UNet11.pt')
       break
